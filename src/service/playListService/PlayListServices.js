...
const AuthorizationsError = require('../../helper/exception/AuthorazationsError');
const ForbiddenError = require('../../helper/exception/Forbiden');
...
class PlaylistsService {
  ...
  async deletePlaylistById(id) {
    ...
    if (!result.rows.length) {
      throw new ForbiddenError('Playlists not found');
    }
  }
  ...
  async addSongOnPlaylist(playlistId, songId, userId) {
    ...
    if (!addSongResult.rows[0].id || !playlistActivityResult.rows[0].id) {
      ...
      throw new InvariantError('The song failed to be added to the Playlist');
    }
  ...
  async deleteSongOnPlaylist(playlistId, songId, userId) {
    ...
    if (!deleteSongResult.rows.length) {
      await this._pool.query('ROLLBACK');
      throw new ForbiddenError('Playlist failed to delete, Id not found');
    }
  ...
  async verifyPlaylistOwner(playlistId, owner) {
    ...
    if (result.rows[0].owner !== owner) {
      throw new ForbiddenError('You have no right to access this resource');
    }
  }
